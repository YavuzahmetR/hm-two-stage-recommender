    ...
    if model is None:
        ...
        ranked = (
            ...
            .with_columns(
                pl.Series(
                    "score",
                    model.booster_.predict(features),
                )
            )
            .sort(
...
def main():
    ...
    validation = pl.read_parquet(
        PROCESSED_DIR
        / f"candidates_{VALIDATION_DATE}_v6.parquet"
    )
    validation_actual = pl.read_parquet(
        PROCESSED_DIR
        / f"actual_{VALIDATION_DATE}_v6.parquet"
    )
    ...
    for candidates in [
        *[frame for _, frame, _ in snapshots],
        validation,
    ]:
        missing = set(FEATURES) - set(candidates.columns)
        if missing:
    ...
    output_dir = (
        DATA_DIR.parent.parent.parent / "reports" / "metrics"
    )
    output_dir.mkdir(parents=True, exist_ok=True)
    report.write_csv(output_dir / "multiweek_v6.csv")
    ...
    print("\nSaved ranker_v6.txt and multiweek_v6.csv")
